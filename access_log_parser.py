...
from nginx_access_log_parser import parse_access_log, parse_request	# parser for nginx access log
from processor import show_details, new_progress, set_initial_timestamp, set_total_time, set_threadlock, set_avg_bytes_sent, set_requests_per_second						# processing and summary of access logs
...
def main():
	...
	while flag:
		...
		if len(lines) != 0:
			# print(lines)
			t = threading.Thread(target=new_progress, args=(lines,))
			t.start()
			threads.append(t)
