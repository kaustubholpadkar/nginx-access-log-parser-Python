...
def summary(records):
	...
	progress_thread = threading.Thread(target=progress, args=())			# thread for displaying the processing progress
	progress_thread.start()													# start the thread

	processor(records)														# processes the records

	progress_thread.join()													# join progress_thread when records are processed
...
def progress():
	...
	# open curses terminal
	stdscr = curses.initscr()
	curses.noecho()
	curses.cbreak()

	threadLock.acquire()													# acquire the lock

	try:
		# print the progress details until the parsing is completed
		while(not parsed):
			stdscr.addstr(0, 0, "Summary :")								# print summary
			stdscr.addstr(1, 0, "Number of requests processed : {0}".		# print no of requests_processed
										format(requests_processed))
			stdscr.addstr(2, 0, "Total time: {0} sec".						# print total_time
										format(total_time))
			stdscr.addstr(3, 0, "Request/sec: {0} req/sec".					# print requests_per_second
